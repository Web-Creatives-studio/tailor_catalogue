...
export default function EditDress({ dress, setEditDress, onDressUpdated }) {
  const [categories, setCategories] = useState([]);
  const [loadingCategories, setLoadingCategories] = useState(true);

  // Form State initialized with selected dress details
  const [form, setForm] = useState({
    title: dress?.title || "",
    categoryId: dress?.categoryId || dress?.category?.id || "",
    price: dress?.price || "",
    completionTime: dress?.completionTime || "5 - 7 working days",
    description: dress?.description || "",
  });

  // Array of image data URLs or links (up to 4) prefilled with current dress images
  const [imagePreviews, setImagePreviews] = useState(dress?.images || []);
    ...
    async function loadCategories() {
      try {
        ...
        if (res.ok) {
          ...
          setCategories(list);
          // Fallback category if none was set
  ...
  const handleSubmit = async (e) => {
    ...
    if (imagePreviews.length === 0) {
      setError("Please select or keep at least 1 image for this outfit.");
      setSubmitting(false);
      return;
    }
    ...
    try {
      ...
      setSuccess("Outfit updated successfully!");

      // Callback to update local UI list if function provided
      if (onDressUpdated) {
        onDressUpdated(data.dress);
      }
    ...
    <div className="fixed inset-0 z-50 flex justify-end bg-black/60 backdrop-blur-sm overflow-hidden font-sans">
      ...
      >
          ...
          <div className="flex items-center gap-2.5">
            ...
            <div>
              ...
              <p className="text-[11px] text-slate-500">
                Modify details or photos for this design
              </p>
            </div>
          </div>
          <button
          ...
          <div className="space-y-3 pt-2 border-t border-slate-100">
            <div className="flex items-center justify-between">
              <label className="block text-xs font-bold uppercase text-slate-700 tracking-wide flex items-center gap-1.5">
                <FiImage className="text-[#128C7E]" /> Select Images (
            ...
            {imagePreviews.length < 4 && (
              <label className="flex flex-col items-center justify-center border-2 border-dashed border-slate-300 hover:border-[#128C7E] bg-slate-50/50 hover:bg-emerald-50/30 rounded-2xl p-5 cursor-pointer transition-colors group">
                ...
                <span className="mt-2 text-xs font-bold text-slate-700">
                  Click to add/replace photos from computer
                </span>
        ...
        <div className="p-5 border-t border-slate-100 bg-white flex items-center justify-end gap-3">
          ...
          >
            {submitting ? "Saving Changes..." : "Update Outfit"}
          </button>
        </div>
      </div>
    </div>
